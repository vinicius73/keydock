...
test.describe("values golden SDK browser coverage", () => {
  ...
  test("covers value roundtrips, key limits, OrNull, and delete semantics", async ({ page }) => {
    ...
    await expect(page.getByTestId("key-129-bytes")).toHaveText("KeydockError:400");
    await expect(page.getByTestId("value-16384-bytes")).toHaveText("stored");
    await expect(page.getByTestId("value-16385-bytes")).toHaveText("KeydockError:400");
    await expect(page.getByTestId("delete-existing")).toHaveText("false");
    await expect(page.getByTestId("delete-missing")).toHaveText("KeydockError:404");
