...
test.describe("TTL and counters golden SDK browser coverage", () => {
  ...
  test("covers TTL expiry, default TTLs, and counter edge cases", async ({ page }) => {
    ...
    await expect(page.getByTestId("counter-negative")).toHaveText("raw:-3,kind:integer,number:-3");
    await expect(page.getByTestId("counter-add-int")).toHaveText("raw:15,kind:integer,number:15");
    ...
    await expect(page.getByTestId("counter-zero-rejected")).toHaveText("KeydockValidationError");
    await expect(page.getByTestId("counter-nan-rejected")).toHaveText("KeydockValidationError");
    await expect(page.getByTestId("counter-non-numeric")).toHaveText("KeydockError:400");
